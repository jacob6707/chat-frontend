...
function Message({ message }) {
  ...
  return (
    <div
      className={`group/msg relative flex max-w-full items-center gap-2 hover:bg-slate-600/5 ${message?.temp ? "text-slate-400" : ""}`}
    >
      <Modal>
        <Modal.Open opens={message.author._id}>
          <AvatarImage
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-end gap-2">
            <Modal.Open opens={message.author._id}>
              <h2 className="text-base font-semibold hover:cursor-pointer hover:underline">
        ...
        <Modal.Window name={message.author._id}>
          <UserProfile id={message.author._id} />
        </Modal.Window>
      </Modal>
      {canModify && (
        <ul className="absolute -right-4 -top-4 hidden rounded-lg bg-slate-900 text-slate-400 group-hover/msg:flex">
          ...
          <button
            className="p-2 first:rounded-l-lg last:rounded-r-lg hover:bg-red-600/75 hover:text-slate-50"
